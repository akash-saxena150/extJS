...
Ext.define('Ext.aria.FocusManager', {
    ...
    constructor: function(config) {
        ...
        me.keyNav = new Ext.util.KeyNav(doc, {
            disabled: true,
            scope: me,
    ...
    disable: function() {
        ...
        me.keyNav.disable();
        me.toggleKeyMap.disable();

        Ext.un('afterlayout', me.reDrawFocus, me);
    ...
    enable: function(options) {
        ...
        if (!cmp) {
            Ext.log.error('Failed to initialize ARIA support. No viewport or component with ariaRole of "application" was found.')
        ...
        // Track layout changes so we can redraw the focus frame
        Ext.on('afterlayout', me.reDrawFocus, me);
    ...
    initDOM: function(options) {
        ...
        // Create global focus frame to display on top of the focused element
        // TODO: set the z index properly
    ...
    removeDOM: function() {
        ...
        // If we are still enabled globally
    ...
    navigateOut: function(e, cmp) {
        ...
        // If the focus component is a field with a tool tip showing, close the tool tip
        if (Ext.AriaSupport.getErrorTipOwner() === focusedCmp) {
            Ext.AriaSupport.hideErrorTip();
    ...
    onComponentFocus: function(cmp, e) {
        ...
        if (!cmp.getFocusFrameEl()) {
            Ext.log.warn("onComponentFocus was called, but the component is not focusable " + cmp.id);
        ...
        me.focusedCmp = cmp;
        me.focusedFrameEl = cmp.getFocusFrameEl();
        me.focusedFrameEl = me.focusedFrameEl.dom ? me.focusedFrameEl : me.focusedFrameEl.el;

        me.focusTask.delay(10, null, null, [cmp, me.focusedFrameEl]);
    ...
    handleComponentFocus: function(cmp, focusEl, suspendEvent) {
        var me = this,
            cls, ff, box, bt, bl, bw, bh, ft, fb, fl, fr;

        if (!focusEl || !focusEl.dom) {
            // because this task is delays always check to make sure the element was not removed
            return;
        }
        
        if (cmp.isContainer) {
            cmp.getTargetEl().scrollIntoView();
        }
        ...
        if (!suspendEvent) {
            Ext.globalEvents.fireEvent('componentfocus', me, cmp, me.previousFocusedCmp);
        }
    },
    ...
    reDrawFocus: function(delay) {
        var me = this,
            focusedCmp = me.focusedCmp,
            focusedFrameEl = me.focusedFrameEl,
            curBox = me.currentBox || (me.currentBox = {}),
            focusFrame = me.focusFrame,
            box;
        
        if (!focusedFrameEl || !focusedFrameEl.dom) {
            return;
        }
        
        box = focusedFrameEl.getPageBox();
        
        if (!box) {
            return;
        }
        
        if (box.left != curBox.left || box.top != curBox.top ||
            box.width != curBox.width || box.height != curBox.height) {
            focusFrame.hide();
            
            if (delay) {
                me.focusTask.delay(10, null, null, [focusedCmp, focusedFrameEl, true]);
            }
            else {
                me.handleComponentFocus(focusedCmp, focusedFrameEl, true);
            }
        }
    },
